import os
import random
import neat_cart_pole as cart_pole
...
def eval_genomes(genomes, config):
    for genome_id, genome in genomes:
        ...
        #genome.fitness = cartPole.getScore(net)
        
        total = 0
        for i in range(100):
            total = total + cartPole.getScore(net)
...
def run(config_file):
    ...
    winner_fitness = cartPole.getScore(winner_net)
    print(f"best fitness = {winner_fitness}")

    best_net = neat.nn.FeedForwardNetwork.create(winner, config)
    cartPole.saveParams(best_net)
